...
std::string lowerString(const std::string& str); //Met une chaine de charactÀre en minuscule
...
string to_JSON(const bool success,const int size,const  int time, const std::vector<Page> &pages,const  std::vector<int> secundaries)
{
    ...
    // On convertit d'abord les donnÈes en string
    ...
    // On crÈe le premier tableau de pages
    output += "\"results\": [";
    ...
    // On crÈe ensuite le tableau des ID restantes
    output += "\"secundaries\": [";
...
int nbWord(const std::string& chaine,const std::string& word,const int& level)
{
    ...
    else
    {
        ...
        for(i = 0; chaineLow[i] != '\0'; i++) //on parcour la chaine
        {
            ...
            {
                if(chaineLow[i] == wordLow[iw]) //Si la premiÀre lettre est identique
                {
                    chaineBin[i] = true;
...
void searchDichotomique(const int& sizeTab,const int *score,const int& value)
{
    ...
    //Si il y a des doublons de valeur, on recherche la place de la premiÀre valeur aparraissante
